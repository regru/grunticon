...
(function( exports ){
	...
	Gfile.prototype.getCSSRules = function( stats, pngfolder, cssprefix, config ){
		...
		var im = config.importantRules ? ' !important' : '';
		...
		var getPNGDataCSSRule = function( prefix, pngdatauri, relPngLocation ){
			if (pngdatauri.length <= 32768) {
				// create png data URI
				return iconsel + " { background-image: url('" +  pngdatauri + "')" + im + "; background-repeat: no-repeat" + im + "; }";
			} else {
				return "/* Using an external URL reference because this image would have a data URI of " +
					...
					iconsel + " { background-image: url('" + relPngLocation.split( path.sep ).join( "/" ) + "')" + im + "; background-repeat: no-repeat" + im + "; }";

			}
		}; //getPNGDataCSSRule

		res.pngcssrule = iconsel + " { background-image: url(" + pngfolder.split( path.sep ).join( "/" ) + this.filenamenoext + ".png" + ")" + im + "; background-repeat: no-repeat" + im + "; }";
		res.htmlmarkup = '<pre><code>.' + prefix + ':</code></pre><div class="' + prefix + '" style="width: '+ stats.width +'; height: '+ stats.height +'"></div><hr/>';
		res.datacssrule = iconsel + " { background-image: url('" + ( this.isSvg ? this.svgdatauri() : this.pngdatauri() ) + "')" + im + "; background-repeat: no-repeat" + im + "; }";
